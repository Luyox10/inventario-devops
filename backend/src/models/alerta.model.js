...
/**
 * Obtiene productos con stock bajo o próximos a vencer
 */
...
async function listProductosVencidos() {
  const sql = `SELECT p.id,
    p.nombre,
    p.sku,
    p.precio,
    ...
    WHERE p.activo = 1 AND COALESCE(l.min_expiry, p.expiry_date) IS NOT NULL AND COALESCE(l.min_expiry, p.expiry_date) < CURRENT_DATE()
    ORDER BY COALESCE(l.min_expiry, p.expiry_date) ASC, p.id DESC`;

  const [rows] = await pool.query(sql);
  return rows;
}

module.exports = { listStockBajo, listProductosVencidos };
